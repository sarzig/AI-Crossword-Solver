...
Some basic tests for feature extraction from clue text.
...
def test_uppercase_percentage():
    def assert_with_print(actual, expected, clue):
        if isinstance(expected, float):
            assert float_equal(actual, expected), (f"FAILED: uppercase_percentage('{clue}') → "
    ...
    assert_with_print(uppercase_percentage("___ Burgundy, Will Ferrell persona"), 3/5,
                      "___ Burgundy, Will Ferrell persona")
    assert_with_print(uppercase_percentage("Action done while saying 'Good dog'"), 2/6,
