...
Some basic tests for testing information retrieval from encyclopedia.

*Gen AI Assisted*
"""

from clue_classification_and_processing.fill_in_the_blank_sandbox import fill_in_the_blank_with_possible_source, \
    preprocess_lower_remove_punct_strip_whitespace
from clue_classification_and_processing.helpers import process_text_into_clue_answer
...
def test_preprocess_text():
    ...
    texas_page_text = rf""" The streak included such songs as "Ocean Front Property", \
    "All My Ex's Live in Texas", "Famous Last Words of a Fool", and "Baby Blue". Strait \
    finished the decade by winning the CMA Entertainer of the Year award in 1989. \
    One year later, he won the award again.[31]"""
...
def test_fill_in_the_blank_with_possible_source():
    ...
    texas_page_text = rf""" The streak included such songs as "Ocean Front Property", "All My Ex's Live in Texas",\
     "Famous Last Words of a Fool", and "Baby Blue". Strait finished the decade by winning the CMA Entertainer of \
     the Year award in 1989. One year later, he won the award again.[31]"""

    assert fill_in_the_blank_with_possible_source(clue_text,
                                                  possible_source_text) == "whose"
    assert fill_in_the_blank_with_possible_source(clue_text,
                                                  possible_source_short) == "whose"
    assert fill_in_the_blank_with_possible_source(clue_text_blank_at_beginning,
                                                  possible_source_text) == "whose"
    assert fill_in_the_blank_with_possible_source(clue_text_blank_at_end,
                                                  possible_source_text) == "soul"
    assert fill_in_the_blank_with_possible_source(clue_text_blank_at_end2,
                                                  possible_source_text) == "soul"
    assert fill_in_the_blank_with_possible_source('the quote goes as: "hello ___ lady"',
                                                  "hello pretty lady") == "pretty"
    assert fill_in_the_blank_with_possible_source('the quote goes as: "hello ___ ___ ___ lady"',
                                                  "hello pretty pretty pretty lady") == "pretty"
    assert fill_in_the_blank_with_possible_source(clue_exes_in_texas,
                                                  texas_page_text) == "exs"
