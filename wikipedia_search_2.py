...
def search_wikipedia_articles(keywords, original_words):
    ...
    search_query = " ".join(keywords[:7])  # Use up to 7 keywords for broader search
    ...
    if response.status_code == 200:
        ...
        for entry in search_results:
            ...
            if page.exists():
                ...
                # Ensure BOTH original words appear in the content OR at least 3 related terms
                word_matches = [word.lower() in page_text for word in original_words]
                if sum(word_matches) >= 2:  # Allow at least 2 related words to appear
                    results[page_title] = page.text  # Store only relevant pages
...
def main():
    ...
    # Find the Wikipedia page that mentions both words the most
    if page_word_counts:
        most_relevant_page = max(page_word_counts, key=page_word_counts.get)
    else:
    ...
    print("\nPage that mentions both words the most:")
    if most_relevant_page:
        print(f"Page: {most_relevant_page} - Total Word Mentions: {page_word_counts[most_relevant_page]}")
    else:
        print("No relevant Wikipedia pages found.")

    if word_length:
        print(f"\nAll Wikipedia pages retrieved (exact title length of {word_length} characters):")
    else:
        print("\nAll Wikipedia pages retrieved (no length restriction):")
